...
output "private_instance_ids" {
  ...
  value       = [for m in values(module.ec2_private) : m.id]
}

# Private EC2 Instances Private IPs
output "private_instance_private_ips" {
  description = "Private EC2 instance private IPs"
  value       = [for m in values(module.ec2_private) : m.private_ip]
}
...
output "private_instance_public_ips" {
  ...
  value       = [for m in values(module.ec2_private) : m.public_ip if m.public_ip != null]
}
